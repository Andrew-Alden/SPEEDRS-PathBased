...
class Trainer(object):
    ...
    def __init__(self, in_dims, model, num_epochs=1000, lr=1e-3):
        ...
        self.optim = optim.Adadelta(self.model.parameters(), lr=lr)  # ,weight_decay=1e-2)
    ...
    def fit(self, train, test=None):
        ...
        for j in range(self.num_epochs):
            ...
            for X, y in train_iterator:
                ...
                y_pred = self.model(Variable(X).cuda()).reshape(-1).cuda()
                loss = self.l(y_pred, Variable(y).cuda()).cuda()
    ...
    def evaluate(self, test, return_all=False):
        ...
        for X, y in test_iterator:
            ...
            # print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
            # print(y_pred.shape)
            # print(Variable(y).shape)
            # print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
            sum_mse += self.l(y_pred, Variable(y).cuda()).data.cuda()
...
class DeepSetRNN(nn.Module):
    ...
    def __init__(self, in_features, set_features=50):
        ...
        self.feature_extractor = nn.RNN(input_size=in_features, hidden_size=set_features, num_layers=1,
                                        batch_first=True)
...
class DeepSetLSTM(nn.Module):
    ...
    def init_hidden(self, batch_size):
        return torch.zeros(1, batch_size, self.out_features).cuda(), torch.zeros(1, batch_size,
                                                                          self.out_features).cuda() #.cuda()
